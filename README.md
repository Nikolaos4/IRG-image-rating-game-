...
1. В папке `api` самостоятельно создайте файл .env (перед . ничего писать не нужно). В файле `api/.env.sample` находятся объявление необходимых переменных, их надо скопировать и заполнить.<br>
**__Заполнение переменных в .env:__**<br>
1.1 В JWT_SECRET не писать что-то в ковычках -> приведет к ошибке.<br>
1.2 На своем устройстве откройте pgAdmin и сформируйте там базу данных, затем создайте к ней ссылку по следующему шаблону: postgresql://USER:PASSWORD@HOST:PORT/DATABASE<br>
    ...
    **В DATABASE** пишите название созданной базы данных<br>
    Затем получившуюся ссылку надо списать в DATABASE_URL в `""` <- таких ковычках.
...
3. запустите миграции (перед этим надо обязательно создать базу данных, иначе приведет к ошибке):
    ...
    ```
4. Заполняет базу данных тестовыми данными:
    ```
    npm run seed
    ```
5. Запустите сервер:
...
(Следующее делать в отдельном терминале)<br>
