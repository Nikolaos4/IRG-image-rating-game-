...
1. В папке `api` самостоятельно создайте файл `.env` (перед `.` ничего писать не нужно). В файле `api/.env.sample` находятся объявления необходимых переменных, их надо скопировать и заполнить.<br>
   **_Заполнение переменных в .env:_**<br>
   1.1 В JWT_SECRET не писать что-то в кавычках -> приведет к ошибке.<br>
   ...
   **В DATABASE** пишите название созданной базы данных<br>
   Затем получившуюся ссылку надо вписать в DATABASE_URL в `""` <- таких кавычках.
   1.3 В TG_TOKEN надо вписать токен, который вы получили от BotFather в Telegram при создании бота.
...
6. В отдельном терминале запустите telegram-бота:
    ```
    npm run bot
    ```
